...
func applyRoundedCorners(hwnd uintptr, w, h, radius int) {
	...
	if ret != 0 {
		...
		// DWM handles both visual clipping and hit-testing on Windows 11+.
		// Skip the GDI region which would add imprecise aliased clipping
		// on top of DWM's smooth anti-aliased corners.
		logger.Debug("applyRoundedCorners: DWM rounded corners applied, skipping GDI", "hwnd", hwnd)
		return
	}

	// Windows 10 fallback: use a GDI rounded-rect region.
